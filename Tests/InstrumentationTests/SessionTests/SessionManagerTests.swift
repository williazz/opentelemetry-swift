...
final class SessionManagerTests: XCTestCase {
  ...
  func testCustomSessionSampleRate() {
    ...
    sessionManager = SessionManager(configuration: SessionConfig(sessionSampleRate: customSampleRate))
  ...
  func testSessionSamplingWithFullSampleRate() {
    sessionManager = SessionManager(configuration: SessionConfig(sessionSampleRate: 1.0))
    sessionManager.getSession()
    XCTAssertTrue(sessionManager.isSessionSampled)
  }

  func testSessionSamplingWithZeroSampleRate() {
    sessionManager = SessionManager(configuration: SessionConfig(sessionSampleRate: 0.0))
    sessionManager.getSession()
  ...
  func testSessionSamplingChangesOnNewSession() {
    sessionManager = SessionManager(configuration: SessionConfig(sessionTimeout: 0, sessionSampleRate: 1.0))
    sessionManager.getSession()
