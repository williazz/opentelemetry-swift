...
final class NetworkSpanTests: XCTestCase {
  ...
  // Only the requests the scenario makes against the mock collector's
  ...
  private func span(forStatus code: Int) throws -> OTLPSpan {
  ...
  func testSpanAttributes() throws {
    for code in Scenario.statusCodes {
      ...
      XCTAssertNotNil(attributes.int("net.peer.port"))
      XCTAssertEqual(attributes.string("http.url"), "http://localhost:\(attributes.int("net.peer.port") ?? 0)/status/\(code)")
  ...
  func testSpanStatusReflectsHTTPStatus() throws {
    ...
    XCTAssertNil(ok.status?.code)
    XCTAssertNil(ok.status?.message)

    let notFound = try span(forStatus: 404)
    XCTAssertEqual(notFound.status?.code, "STATUS_CODE_ERROR")
    XCTAssertEqual(notFound.status?.message, "404")

    let serverError = try span(forStatus: 500)
    XCTAssertEqual(serverError.status?.code, "STATUS_CODE_ERROR")
